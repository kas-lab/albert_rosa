...
private:
  ...
  int max_runs_ = 100;
  int current_run_ = 0;
  rclcpp::Publisher<std_msgs::msg::Int32>::SharedPtr benchmark_pub_;
  ...
  struct BenchmarkRunData {
    ...
    // Adaptations
    int num_config_adaptations;
    int num_component_adaptations;
    std::vector<std::string> adaptation_events;
    
    // Config usage (seconds)
    std::map<std::string, double> config_usage_time;
    
    // Map characteristics
    int total_waypoints;
    int total_corridors;
    int narrow_corridors;
    int medium_corridors;
    int wide_corridors;
    int num_chargers;
    std::vector<std::string> charger_locations;
    
    // Performance
    double avg_speed;
    int emergency_events;
    
    // Predictions
    std::vector<double> predicted_batteries;
    std::vector<double> actual_batteries;
  };
  // ✅ ADD:
  bool perception_active_ = true;
  bool arm_active_ = true;
  std::string target_charger_ = "";
  
  // ✅ ADD:
  void fetch_component_states();
  BenchmarkRunData current_run_data_;
  std::vector<BenchmarkRunData> all_run_data_;
  rclcpp::Publisher<std_msgs::msg::String>::SharedPtr benchmark_data_pub_;
  
  // Function declaration
  void publishBenchmarkData();
  void initializeBenchmarkRun();



  void addCorridorTraversalRules(const std::vector<std::string>& feasible_cfgs);
  std::mt19937 rng_;  // Random number generator
  int map_seed_ = 103;
  

  bool isGraphConnected(int num_nodes, const std::vector<Edge>& edges);
  ...
  rclcpp::Publisher<diagnostic_msgs::msg::DiagnosticArray>::SharedPtr monitoring_pub_;   
...
protected:
  ...
  void deleteMapFromKB();
  void generateAndInjectMap();
  void initializeCorridorWidthInKB(double default_width);
  ...
  void updatePredictedBatteryInKB(double predicted_level, const std::string& reason = "battery_update");
